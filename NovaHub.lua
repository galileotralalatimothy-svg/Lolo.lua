if (game:GetService("CoreGui")):FindFirstChild("NOVA HUB") and (game:GetService("CoreGui")):FindFirstChild("ScreenGui") then
	(game:GetService("CoreGui")).NOVA HUB:Destroy();
	(game:GetService("CoreGui")).ScreenGui:Destroy();
end;
_G.Primary = Color3.fromRGB(140, 140, 140);
_G.Dark = Color3.fromRGB(18, 18, 20);
_G.Third = Color3.fromRGB(0, 200, 120);
...
ImageButton.MouseButton1Click:connect(function()
	(game.CoreGui:FindFirstChild("NOVA HUB")).Enabled = not (game.CoreGui:FindFirstChild("NOVA HUB")).Enabled;
end);
...
(getgenv()).LoadConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("NOVA HUB") then
			makefolder("NOVA HUB");
		end;
		if not isfolder("NOVA HUB/Library/") then
			makefolder("NOVA HUB/Library/");
		end;
		if not isfile(("NOVA HUB/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("NOVA HUB/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(SettingsLib));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("NOVA HUB/Library/" .. game.Players.LocalPlayer.Name .. ".json"));
...
(getgenv()).SaveConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfile(("NOVA HUB/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			...
			writefile("NOVA HUB/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
		end;
	else
		return warn("Status : Undetected Executor");
	end;
...
function Update:Window(Config)
	...
	local NOVA HUB = Instance.new("ScreenGui");
	NOVA HUB.Name = "NOVA HUB";
	NOVA HUB.Parent = game.CoreGui;
	NOVA HUB.DisplayOrder = 999;
	local OutlineMain = Instance.new("Frame");
	OutlineMain.Name = "OutlineMain";
	OutlineMain.Parent = NOVA HUB;
	...
	CloseButton.Image = "rbxassetid://130598277531514";
	...
	SearchBar.BackgroundColor3 = Color3.fromRGB(35, 35, 38);
	SearchBar.BackgroundTransparency = 0.2;
	SearchBar.Position = UDim2.new(0, 10, 0, Top.Size.Y.Offset + 6);
	SearchBar.Size = UDim2.new(0, Tab.Size.X.Offset - 10, 0, 30);
	CreateRounded(SearchBar, 10);

	local SearchText = Instance.new("TextLabel");
	SearchText.Parent = SearchBar;
	SearchText.BackgroundTransparency = 1;
	SearchText.Position = UDim2.new(0, 10, 0.5, 0);
	SearchText.AnchorPoint = Vector2.new(0, 0.5);
	SearchText.Size = UDim2.new(1, -20, 1, 0);
	SearchText.Font = Enum.Font.Gotham;
	SearchText.Text = "Search";
	SearchText.TextColor3 = Color3.fromRGB(170, 170, 170);
